...
msg=$.validity.settings.requireMsg;return validate(this,function(obj){return obj.value.length>0;},msg);};$.fn.match=function(rule,msg){if(msg==null){msg=$.validity.settings.matchMsg;if(typeof(rule)=="string"&&$.validity.settings[rule+"Msg"]!=null)
...
msg=$.validity.settings.tooLongMsg;return validate(this,function(obj){return obj.value.length<=max;},msg);};$.fn.equal=function(arg0,arg1){if(this.length>0){var transform=function(val){return val;};var msg=$.validity.settings.equalMsg;if(typeof(arg0)=="function"){transform=arg0;if(typeof(arg1)=="string")
msg=arg1;}
...
return $phi;};$.fn.distinct=function(arg0,arg1){if(this.length>0){var transform=function(val){return val;};var msg=$.validity.settings.distinctMsg;if(typeof(arg0)=="function"){transform=arg0;if(typeof(arg1)=="string")
msg=arg1;}
else if(typeof(arg0)=="string")
msg=arg0;var values=new Array();var valid=true;this.each(function(idx){var transformedValue=transform(this.value);for(i in values){if(transformedValue.length>0&&values[i]==transformedValue)
valid=false;}
values[idx]=transformedValue;});if(valid)
return this;raiseAggregateError(this,msg);}
return $phi;};$.fn.sum=function(sum,msg){if(msg==null)
msg=$.validity.settings.sumMsg;if(this.length>0&&sum==numericSum(this))
return this;return $phi;};$.fn.sumMax=function(max,msg){if(msg==null)
msg=$.validity.settings.sumMsg;if(this.length>0){if(max>=numericSum(this))
return this;raiseAggregateError(this,msg);}
...
function raiseLabelError(obj,msg){var $obj=$(obj);var errorId=$obj.attr("id");var errorSelector="#"+errorId;var labelSelector="label[for='"+errorId+"']";if($(labelSelector).length==0)
$("<label/>").attr("for",errorId).addClass("error").text(msg).insertAfter(errorSelector);else
$(labelSelector).text(msg);}
function raiseModalError(obj,msg){var $obj=$(obj);var off=$obj.offset();var errorStyle={left:parseInt(off.left+$obj.width()+4)+"px",top:parseInt(off.top-10)+"px"};var errorId=prefixes.modalErrorId+$obj.attr("id");var errorSelector="#"+errorId;if($(errorSelector).length==0)
$("<div/>").attr("id",errorId).addClass(classes.modalError).css(errorStyle).text(msg).click($.validity.settings.modalErrorsClickable?function(){$(this).remove();}:null).appendTo(selectors.modalOutput);else
...
function raiseAggregateLabelError(obj,msg){if(obj.length>0)
